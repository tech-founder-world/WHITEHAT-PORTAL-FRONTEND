...
function StudentProfile({
  ...
  useEffect(() => {
        ...
        if (isAdmin) {
          ...
          const attResults = await Promise.all(
            teacherSubjects.map((sub) =>
              api
                .get(
                  `/attendance?studentId=${student._id}&subject=${encodeURIComponent(sub)}`,
                )
                .then((r) => r.data)
                .catch(() => []),
            ),
          ...
          const evalResults = await Promise.all(
            teacherSubjects.map((sub) =>
              api
                .get(
                  `/evaluations?studentId=${student._id}&subject=${encodeURIComponent(sub)}`,
                )
                .then((r) => r.data)
                .catch(() => []),
            ),
    ...
    load();
  }, [student._id, isAdmin, isCounsellor, teacherSubjects]);
            ...
            {subjectStats.map((s) => {
              ...
              return (
                <div key={s.subject} className="profile-stat-chip">
                  ...
                  <div style={{ color, fontWeight: 800, fontSize: "18px" }}>
                    {s.pct}%
                  </div>
                ...
                return (
                  <div key={sub} style={{ marginBottom: "20px" }}>
                    ...
                    >
                      ...
                      <span
                        style={{ fontWeight: 700, color, fontSize: "13px" }}
                      >
                        {pct}% · {present}/{records.length}
                      </span>
                    </div>
                    <div
                      style={{
                      ...
                      {records.map((r) => (
                        <div
                          ...
                          style={{
                            ...
                            background:
                              r.status === "present" ? "#F1F8E9" : "#FFF8F8",
                          }}
                        >
                          <span
                            style={{
                          ...
                          {editRecord === r._id ? (
                            ...
                            >
                              <button
                                ...
                                onClick={() => handleEditStatus(r, "present")}
                              >
                                Present
                              </button>
                              <button
                                className="btn btn-danger btn-sm"
                                onClick={() => handleEditStatus(r, "absent")}
                              >
                                Absent
                              </button>
                              <button
          ...
          <div className="profile-scroll">
            {evaluations.length === 0 ? (
              ...
              Object.entries(
                ...
                const avg = (
                  evals.reduce((s, e) => s + (e.score / e.maxScore) * 100, 0) /
                  evals.length
                ).toFixed(1);
                const avgColor =
                  parseFloat(avg) >= 70
                      ...
                      {evals.map((e) => {
                        ...
                        return (
                          <div
                            ...
                            style={{
                              display: "grid",
                              gridTemplateColumns: "90px 1fr 60px 1fr",
                              gap: "10px",
                              alignItems: "center",
                              padding: "9px 14px",
                              borderBottom: "1px solid var(--gray-100)",
                          ...
                          >
                            ...
                            <span
                              style={{
                                fontSize: "12px",
                                color: "var(--gray-600)",
                                fontStyle: e.remarks ? "normal" : "italic",
                              }}
                            >
...
export default function AttendanceHistory() {
  ...
  useEffect(() => {
        ...
        if (isAdmin) {
          ...
          if (teacherSubjects.length > 0) {
            const results = await Promise.all(
              teacherSubjects.map((sub) =>
                api
                  .get(`/students?subject=${encodeURIComponent(sub)}`)
                  .then((r) => r.data || [])
                  .catch(() => []),
              ),
            );
    ...
    load();
  }, [isAdmin, isCounsellor, teacherSubjects]);
      ...
      {loading ? (
        ...
        <div className="empty-state card">
          ...
          {students.length === 0 && (
            ...
            >
              {isAdmin
                ? "Go to Students to add students."
                : isCounsellor
            ...
            return (
              ...
              >
                ...
                <div className="spc-stat">
                  <div className="spc-pct" style={{ color }}>
